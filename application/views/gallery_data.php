...
// 生成数据数组
function getPicArr($picSize = '80x60', $pageNo = 1, $pageSize = 10, $total = 5000) {
    $itemArr = array();
    for ($i = 1; $i <= $pageSize; $i++) {
        ...
        if (!$picWidth) 
        	$picWidth = is_null($picHeight) ? '80' : $picHeight;
        if (!$picHeight) 
        	$picHeight = is_null($picWidth) ? '60' : $picWidth;
...
$picSize = is_null($picSize) ? '80x60' : $picSize; // 图片尺寸，默认：80x60
